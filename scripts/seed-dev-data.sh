...
# `DB_DATABASE=SomeOtherDb bash scripts/seed-dev-data.sh` seed the database named in .env instead. On a
...
# unchanged behaviour, so the ordinary `bash scripts/seed-dev-data.sh` still reads .env exactly as before.
