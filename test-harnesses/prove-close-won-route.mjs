 ...
 */
/**
 * TASKS' ENTITIES, added when this harness was merged onto a branch where the close CREATES TASKS.
 *
 * Without them `MJ_BizApps_Tasks: Tasks` resolves through the ClassFactory to a bare `BaseEntity`, the
 * task save fails validation, and the close reports `The OrderReview task could not be created` — a
 * warning that is entirely an artifact of what this script did not load. It read exactly like a real
 * defect in the task service, which is the same shape as KI-21: absence looks like breakage.
 *
 * Entities only, not `tasks-entities-server`, matching how the integration runner loads it — the
 * service supplies everything the row needs, and pulling in a second server package here would widen
 * the process for no gain.
 */
await importSibling('@mj-biz-apps/tasks-entities').then((m) => m.LoadGeneratedEntities?.());
await importSibling('@mj-biz-apps/orders-core-entities-server').then((m) => m.LoadOrdersCoreEntitiesServer?.());
await importSibling('@mj-biz-apps/contracts-core-entities-server').then((m) =>
    m.LoadMjBizappsContractsEntitiesServer?.(),
);
await importSibling('@mj-biz-apps/sales-core-entities-server').then((m) => m.LoadSalesCoreEntitiesServer?.());

const { CloseDealOperation } = await importSibling('@mj-biz-apps/sales-core-entities-server');
