-- BizAppsSales: Add Predictive Deal Win Outcome Columns & Engineered View Features
-- Materialized columns for Predictive Studio deal win forecasting and operational sorting/filtering.
...
---------------------------------------------------------------------------
-- 2. Update vwDeals: include engineered training features
---------------------------------------------------------------------------
CREATE OR ALTER VIEW [${flyway:defaultSchema}].[vwDeals]
AS
SELECT
    d.*,
    ...
    END AS [WinOutcome],
    CASE 
        WHEN d.ExpectedCloseDate IS NOT NULL AND d.StartDate IS NOT NULL THEN DATEDIFF(day, d.StartDate, d.ExpectedCloseDate) 
        WHEN d.ExpectedCloseDate IS NOT NULL THEN DATEDIFF(day, CAST(d.__mj_CreatedAt AS date), d.ExpectedCloseDate) 
        ELSE NULL 
    END AS [DaysToExpectedClose],
    CASE 
        WHEN EXISTS(SELECT 1 FROM [${flyway:defaultSchema}].[DealPaymentSchedule] ps WHERE ps.DealID = d.ID) THEN 1 
        ELSE 0 
    END AS [HasPaymentSchedule],
    ISNULL((SELECT COUNT(*) FROM [${flyway:defaultSchema}].[DealTeamMember] tm WHERE tm.DealID = d.ID), 0) AS [TeamMemberCount],
    CASE 
        WHEN EXISTS(SELECT 1 FROM [${flyway:defaultSchema}].[DealTeamMember] tm JOIN [${flyway:defaultSchema}].[DealRole] r ON tm.DealRoleID = r.ID WHERE tm.DealID = d.ID AND r.Code = 'PARTNERMGR') THEN 1 
        ELSE 0 
    END AS [HasPartnerInvolved],
    CASE WHEN d.Amount >= 100000 THEN 1 ELSE 0 END AS [IsEnterpriseTier],
    CASE WHEN d.AutoRenew = 1 THEN 1 ELSE 0 END AS [AutoRenewFlag],
    CASE WHEN d.StandardAgreementModified = 1 THEN 1 ELSE 0 END AS [StandardAgreementModifiedFlag]
FROM
...
---------------------------------------------------------------------------
-- 3. Register virtual fields in [${mjSchema}].[EntityField] for Predictive Studio
---------------------------------------------------------------------------
DECLARE @entityID UNIQUEIDENTIFIER = '79148DE5-7F99-44AC-ACD4-5EE7BA93D354';
...
/* SQL text to insert 3 new entity field(s) */
      ...
      IF NOT EXISTS (SELECT 1 FROM [${mjSchema}].[EntityField] WHERE ID = 'f3e8b282-249c-4817-be3d-00efad3569d3' OR (EntityID = '79148DE5-7F99-44AC-ACD4-5EE7BA93D354' AND Name = 'PredictedWinProbability')) BEGIN
         ...
         VALUES
         (
            'f3e8b282-249c-4817-be3d-00efad3569d3',
      ...
      IF NOT EXISTS (SELECT 1 FROM [${mjSchema}].[EntityField] WHERE ID = '7c778bdc-ec7e-4275-b8be-49792d542d0b' OR (EntityID = '79148DE5-7F99-44AC-ACD4-5EE7BA93D354' AND Name = 'PredictedWinRiskBand')) BEGIN
         INSERT INTO [${mjSchema}].[EntityField]
         (
            [ID],
            [EntityID],
         ...
         VALUES
         (
            '7c778bdc-ec7e-4275-b8be-49792d542d0b',
            '79148DE5-7F99-44AC-ACD4-5EE7BA93D354', -- Entity: MJ_BizApps_Sales: Deals
            (SELECT COALESCE(MAX([Sequence]), 0) + 1 FROM [${mjSchema}].[EntityField] WHERE [EntityID] = '79148DE5-7F99-44AC-ACD4-5EE7BA93D354'),
            'PredictedWinRiskBand',
            'Predicted Win Risk Band',
      ...
      IF NOT EXISTS (SELECT 1 FROM [${mjSchema}].[EntityField] WHERE ID = 'b1e35f02-c32a-4f17-b0e5-5577a1fd3950' OR (EntityID = '79148DE5-7F99-44AC-ACD4-5EE7BA93D354' AND Name = 'PredictedWinScoredAt')) BEGIN
         INSERT INTO [${mjSchema}].[EntityField]
         (
            [ID],
            [EntityID],
         ...
         VALUES
         (
            'b1e35f02-c32a-4f17-b0e5-5577a1fd3950',
            '79148DE5-7F99-44AC-ACD4-5EE7BA93D354', -- Entity: MJ_BizApps_Sales: Deals
            (SELECT COALESCE(MAX([Sequence]), 0) + 1 FROM [${mjSchema}].[EntityField] WHERE [EntityID] = '79148DE5-7F99-44AC-ACD4-5EE7BA93D354'),
            'PredictedWinScoredAt',
            'Predicted Win Scored At',
...
/* SQL text to insert entity field value with ID 3309e289-23d8-45a5-8d95-42acf3731ed2 */
INSERT INTO [${mjSchema}].[EntityFieldValue]
                                       ([ID], [EntityFieldID], [Sequence], [Value], [Code], [__mj_CreatedAt], [__mj_UpdatedAt])
                                    VALUES
                                       ('3309e289-23d8-45a5-8d95-42acf3731ed2', '7C778BDC-EC7E-4275-B8BE-49792D542D0B', 1, 'Critical', 'Critical', GETUTCDATE(), GETUTCDATE());

/* SQL text to insert entity field value with ID 4b3eaf23-8e1f-40ef-b5d5-4bcfef2d5d6e */
INSERT INTO [${mjSchema}].[EntityFieldValue]
                                       ([ID], [EntityFieldID], [Sequence], [Value], [Code], [__mj_CreatedAt], [__mj_UpdatedAt])
                                    VALUES
                                       ('4b3eaf23-8e1f-40ef-b5d5-4bcfef2d5d6e', '7C778BDC-EC7E-4275-B8BE-49792D542D0B', 2, 'High', 'High', GETUTCDATE(), GETUTCDATE());

/* SQL text to insert entity field value with ID af4843dd-a66f-4519-8c63-7b2eeb02c265 */
INSERT INTO [${mjSchema}].[EntityFieldValue]
                                       ([ID], [EntityFieldID], [Sequence], [Value], [Code], [__mj_CreatedAt], [__mj_UpdatedAt])
                                    VALUES
                                       ('af4843dd-a66f-4519-8c63-7b2eeb02c265', '7C778BDC-EC7E-4275-B8BE-49792D542D0B', 3, 'Low', 'Low', GETUTCDATE(), GETUTCDATE());

/* SQL text to insert entity field value with ID 171d3515-c848-4292-93a6-be355577b3f9 */
INSERT INTO [${mjSchema}].[EntityFieldValue]
                                       ([ID], [EntityFieldID], [Sequence], [Value], [Code], [__mj_CreatedAt], [__mj_UpdatedAt])
                                    VALUES
                                       ('171d3515-c848-4292-93a6-be355577b3f9', '7C778BDC-EC7E-4275-B8BE-49792D542D0B', 4, 'Medium', 'Medium', GETUTCDATE(), GETUTCDATE());
...
LEFT OUTER JOIN
  ...
  ON
    [d].[OrderID] = mjBizAppsOrdersOrderHeader_OrderID.[ID]
GO
REVOKE SELECT ON [${flyway:defaultSchema}].[vwDeals] FROM [cdp_Developer]
