...
/**
 * `mjTip` — a small, reusable, THEMED hover tooltip for workspace surfaces (tab labels, truncated
 * selects, …). Standardized here because MJ has no equivalent directive (confirmed); a candidate to
 * upstream. Behavior (lifted from the tab-strip original so it's identical everywhere):
 ...
 *     glowed in dark). Capped at ~1.5 tab-widths × 3 lines — never a screen-wide banner.
 */
@Directive({ selector: '[mjTip]', standalone: true })
export class WorkspaceTipDirective implements OnDestroy {
