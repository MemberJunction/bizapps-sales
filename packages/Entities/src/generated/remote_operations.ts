...
// ============================================================
// Sales.SaveDeal — Save Deal
// ============================================================
/**
 * Save Deal
 * Create or update a deal together with its lines and its negotiated payment schedule, in ONE transaction. This is the operation that makes browser-side deal entry possible at all: a browser holds the generated DealEntity rather than DealEntityServer, so line and schedule collections are transient properties on the server entity and cannot cross the entity-save boundary as scalars. Saving the header and then each child as separate round trips is the alternative, and a failure partway through it leaves a numbered deal with nothing under it. Prices nothing — every figure it accepts is either an INPUT to the pricing engine or a transcription of what was signed.
 * GenerationType=Manual — the server body is supplied by a hand-authored subclass registered
 * under 'Sales.SaveDeal'. This generated base provides the typed contract only (client-safe).
 */
export class SalesSaveDealOperation extends BaseRemotableOperation<SalesSaveDealInput, SalesSaveDealOutput> {
    public readonly OperationKey = "Sales.SaveDeal";
    public readonly ExecutionMode = 'Sync' as const;
